...
suite('downloadSvg Test Suite', () => {
  ...
  test('downloadSvg should handle timeout', async () => {
    try {
      ...
      assert.ok(error instanceof Error);
      const hasExpectedError = error.message.includes('timeout') ||
  ...
  test(
      'downloadSvg should download Unity Technologies logo and validate it',
      async function() {
    const unityLogoUrl =
    ...
    try {
      ...
      assert.ok(
          ...
          'Downloaded content should be a string',
      );
      assert.ok(
      ...
      assert.ok(
          isValidSvg(downloadedContent),
          'Downloaded Unity logo should be valid SVG',
      );
      assert.ok(
          downloadedContent.includes('<svg'),
          'Downloaded content should contain SVG tag',
      );
      assert.ok(
          downloadedContent.includes('</svg>'),
          'Downloaded content should contain closing SVG tag',
      );
    } catch (error) {
      // Network issues may cause this test to fail in some environments
      console.warn(
          'Unity logo download test failed (possibly due to network issues):',
          error.message,
      );
      // Skip the test if there are network issues but don't fail it
      const hasNetworkError = error.message.includes('timeout') ||
          error.message.includes('ENOTFOUND') ||
          error.message.includes('network');
      if (hasNetworkError) {
