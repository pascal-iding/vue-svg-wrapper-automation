...
const {cleanSvg} = require(
    '../commands/svgToVue/helpers/createSvgWrapperComponent/helpers/helpers' +
    '/getSvgFromClipboard/helpers/extractSvg/helpers/cleanSvg',
);
...
suite('cleanSvg Test Suite', () => {
  ...
  test('cleanSvg should remove both class and id attributes', () => {
    const input =
        '<svg class="icon" id="my-icon" width="24" height="24"></svg>';
    const expected = '<svg width="24" height="24"></svg>';
    assert.strictEqual(cleanSvg(input), expected);
  });

  test('cleanSvg should preserve other attributes', () => {
    const input = '<svg class="icon" width="24" height="24" ' +
        'viewBox="0 0 24 24" fill="none"></svg>';
    const expected = '<svg width="24" height="24" ' +
        'viewBox="0 0 24 24" fill="none"></svg>';
    assert.strictEqual(cleanSvg(input), expected);
  });

  test('cleanSvg should handle SVG without class or id', () => {
    const input = '<svg width="24" height="24">' +
