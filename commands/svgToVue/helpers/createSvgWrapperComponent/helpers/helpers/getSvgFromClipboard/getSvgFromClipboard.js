...
/**
 * Either reads the svg from the clipboard or downloads it from a
 * link.
 * If the downloaded content is a website, it will try to extract an
 * svg from the html.
...
async function getSvgFromClipboard() {
  try {
    ...
    if (!allowDownloadWithHTTPS) {
      throw Error(
          'Downloading from URLs is disabled. Please paste SVG content ' +
          'directly or enable "Allow Download With HTTPS" in settings.');
    }

    try {
      const downloadedContent = (await downloadSvg(trimmedContent)).trim();

      // Clipboard content is direct link to svg file
      if (isValidSvg(downloadedContent)) {
        vscode.window.showInformationMessage(
            'SVG downloaded and validated successfully!');
        return downloadedContent;
      } else {
        // Clipboard is link to generic website
        const extractedSvg = await extractSvg(downloadedContent);
        if (extractedSvg && isValidSvg(extractedSvg)) {
          vscode.window.showInformationMessage(
              'SVG extracted from webpage and validated successfully!');
          return extractedSvg;
          ...
          throw Error(
              'Downloaded content does not contain a valid SVG');
        }
      }
    } catch (downloadError) {
