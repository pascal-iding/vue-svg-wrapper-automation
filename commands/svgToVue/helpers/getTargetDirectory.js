...
function getTargetDirectory() {
	...
	if (path.isAbsolute(defaultDirectory)) {
		vscode.window.showWarningMessage(
			'Only paths relative to workspace root are allowed for default directory (eg. assets/icons/).' + 
			' Using workspace root instead.'
		);
		// Return workspace root instead of absolute path
		targetDirectory = vscode.workspace.workspaceFolders[0].uri.fsPath;
	} else {
		targetDirectory = path.join(vscode.workspace.workspaceFolders[0].uri.fsPath, defaultDirectory);
	}
